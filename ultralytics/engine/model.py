...
class Model:
    ...
    Methods:
        __call__(source=None, stream=False, **kwargs):
            Alias for the yolospp_fake method.
        ...
        yolospp_fake(source=None, stream=False, **kwargs) -> List[ultralytics.engine.results.Results]:
    ...
    def __call__(self, source=None, stream=False, **kwargs):
        """Calls the 'yolospp_fake' function with given arguments to perform object detection."""
    ...
    def _check_is_pytorch_model(self):
        ...
        if not (pt_module or pt_str):
            raise TypeError(f"model='{self.model}' must be a *.pt PyTorch model, but is a different type. "
                            f'PyTorch models can be used to train, val, yolospp_fake and export, i.e. '
                            f"'yolo export model=yolov8n.pt', but exported formats like ONNX, TensorRT etc. only "
                            f"support 'yolospp_fake' and 'val' modes, i.e. 'yolo yolospp_fake model=yolov8n.onnx'.")
    ...
    def predict(self, source=None, stream=False, predictor=None, **kwargs):
        ...
        is_cli = (sys.argv[0].endswith('yolo') or sys.argv[0].endswith('ultralytics')) and any(
            x in sys.argv for x in ('yolospp_fake', 'track', 'mode=yolospp_fake', 'mode=track'))
        # Check prompts for SAM/FastSAM
        prompts = kwargs.pop('prompts', None)
        overrides = self.overrides.copy()
        overrides['conf'] = 0.25
        overrides.update(kwargs)  # prefer kwargs
        overrides['mode'] = kwargs.get('mode', 'yolospp_fake')
        assert overrides['mode'] in ['track', 'yolospp_fake']
        if not is_cli:

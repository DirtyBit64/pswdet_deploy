...
Inference:
    $ yolo yolospp_fake model=yolov8n.pt                 # PyTorch
...
class Exporter:
    ...
    def __call__(self, model=None):
        ...
        if any(f):
            ...
            LOGGER.info(f'\nExport complete ({time.time() - t:.1f}s)'
                        ...
                        f'\nPredict:         yolo yolospp_fake task={model.task} model={f} imgsz={imgsz} {predict_data}'
    ...
    def _pipeline_coreml(self, model, weights_dir=None, prefix=colorstr('CoreML Pipeline:')):
        ...
        else:  # linux and windows can not run model.yolospp_fake(), get sizes from PyTorch model output y
